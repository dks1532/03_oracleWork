...
-- 1) 연결할 컬럼명이 다른 경우(EMPLYEE: DEPT_CODE / DEPARTMENT:DEPT_ID)
--    JOIN ON 구문으로만 가능
...
SELECT EMP_ID, EMP_NAME, BONUS, DEPT_TITLE
FROM EMPLOYEE, DEPARTMENT
WHERE BONUS IS NOT NULL AND DEPT_ID = DEPT_CODE;
...
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_TITLE != '총무부' AND DEPT_ID = DEPT_CODE;
...
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);  -- 23행
...
-- 2) FULL(OUTER) JOIN : 두 테이블이 가진 모든 행 조회(단, 오라클 구문으로는 안됨)
--    >> 오라클전용 구문
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12
FROM EMPLOYEE
...
-- 사번, 사원명, 부서명, 직급명
--  >> 오라클 전용 구문
SELECT EMP_ID, EMP_NAME, DEPT_TITEL, JOB_NAME
