...
/*
    ...
    * 종류 : NOT NULL, UNIQUE, CHECK(조건), PRIMARY KEY(not null + unique), FOREIGN KEY
*/
/*
    * NOT NULL 제약조건
    ...
    제약조건을 부여하는 방식 크기 2가지로 나눌 수 있다(컬럼 레벨 방식/테이블 레벨 방식)
    * NOT NULL은 오로지 컬럼 레벨 방식만 됨
*/
-- 컬럼 레벨 방식 : 컬럼명 자료형 제약조건
CREATE TABLE MEM_NOTNULL (
    MEM_TO NUMBER NOT NULL,
...
INSERT INTO MEM_NOTNULL VALUES(2,'user01',null,'남길동','남',null,'abc@google.com');
-- not null제약조건 위배되는 오류 발생

INSERT INTO MEM_NOTNULL VALUES(1,'user01','pass03','이길순','여',null,'def@google.com');
-- 번호, id는 중복되면 안되는데 잘 추가가 되었음
...
INSERT INTO MEM_UNIQUE2 VALUES(1,'user02','pass02','박길동','남','010-1234-5678','park@naver.com');
INSERT INTO MEM_UNIQUE2 VALUES(2,'user01','pass02','박길동','남','010-1234-5678','park@naver.com');
-- 오류 구문은 제약조건명으로 알려줌.
...
CREATE TABLE MEM_UNIQUE3(
    MEM_NO NUMBER CONSTRAINT MEMNO_MN NOT NULL,
...
INSERT INTO MEM_UNIQUE3 VALUES(2,'USER01','PASS02','이순신',NULL,NULL,NULL);   -- UNIQUE오류
INSERT INTO MEM_UNIQUE3 VALUES(2,'USER02','PASS02','이순신','남',NULL,NULL);
...
INSERT INTO MEM_UNIQUE3 VALUES(1,'USER01','PASS01','아무개',NULL,NULL,NULL);
INSERT INTO MEM_UNIQUE3 VALUES(2,'USER02','PASS02','박무개',NULL,NULL,NULL);

INSERT INTO MEM_UNIQUE3 VALUES(2,'USER03','PASS03','최순신','남',NULL,NULL);
-- > 회원번호를 식별자의 역할
...
CREATE TABLE MEM_PRIKEY2 (
    ...
    CONSTRAINT GENDER_CH2 CHECK(GENDER IN ('남','여')),
    CONSTRAINT MEMNO_PK2 PRIMARY KEY(MEM_NO)
);

INSERT INTO MEM_PRIKEY VALUES(1,'USER01','PASS01','이순신','여',NULL,NULL);
INSERT INTO MEM_PRIKEY VALUES(1,'USER02','PASS02','강순이','여',NULL,NULL);  -- UNIQUE위배
INSERT INTO MEM_PRIKEY VALUES(NULL,'USER02','PASS02','강순이','여',NULL,NULL); -- NOT NULL 위배
...
INSERT INTO MEM_PRIKEY4 VALUES(2,'USER02','PASS02','김동순','여',NULL,NULL);
INSERT INTO MEM_PRIKEY4 VALUES(2,'USER02','PASS04','최동순','여',NULL,NULL);   -- UNIQUE 위배
INSERT INTO MEM_PRIKEY4 VALUES(NULL,NULL,'PASS04','최동순','여',NULL,NULL);    -- NOT NULL위배
...
WHERE GRADE_CODE = 20;
-- 자식테이블에 20이라는 값을 사요하고 있기 때문에 삭제가 안됨

DELETE FROM MEM_GRADE
WHERE GRADE_CODE = 30;
-- 자식테이블에 30이라는 값을 사용하고 있지 않기 때문에 삭제가 됨

-- 자식테이블이 이미 사용하고 있는 값이 있을경우
...
/*
    ...
    * 삭제 옵션 : 부모테이블의 데이터 삭제 시 그 데이터를 사용하고 있는
    ...
    - ON DELETE RESTRICTED(기본값) : 삭제제한옵션으로, 자식테이블이 데이터를 사용하고 있으면 부모데이터 삭제 안됨
    - ON DELETE SET NULL : 자식테이블이 데이터를 사용하고 있으면 자식테이블의 값을 NULL로 변경 부모데이터 삭제
...
-- GRADE_ID NUMBER REFERENCES MEM_GRADE  --> MEM_GRADE테이블의 PRIMARY KEY와 외래키를 걸면 컬럼명을 안써도 됨
...
/*
    <DEFAULT 기본값>
    컬럼을 선정하지 않고 INSERT시 NULL이 아닌 기본값을 INSERT하고자 할때 세팅해 둘 수 있는값
...
AS SELECT *
    FROM EMPLOYEE;
-- 컬럼, 데이터값, 제약조건 같은경우 NOT NULL만 복사됨
-- DEFAULT와 COMMNETS는 COPY안됨

CREATE TABLE EMPLOYEE_COPY2
AS SELECT EMP_ID, EMP_NAME, SALARY, BONUS
    FROM EMPLOYEE
    WHERE 1=0;  -- 구조만 복사하고자 할 때 쓰이는 구문(데이터값이 필요 없을 때)

CREATE TABLE EMPLOYEE_COPY3
AS SELECT EMP_ID, EMP_NAME, SALARY, SALARY*12
    FROM EMPLOYEE;
-- 서브쿼리 SELECT절에 산술식 또는 함수식이 기술된 경우 반드시 별칭 부여해야 함
...
-- EMPLOYEE_COPY 테이블에 DEPT_CODE에 외래키제약조건 추가
ALTER TABLE EMPLOYEE ADD FOREIGN KEY(DEPT_CODE) REFERENCES DEPARTMENT;

-- EMPLOYEE 테이블에 JOB_CODE에 외래키제약조건 추가
ALTER TABLE EMPLOYEE ADD FOREIGN KEY(JOB_CODE) REFERENCES JOB;

-- DEPARTMENT 테이블에 LOCATION_ID에 외래키제약조건 추가(LOCATION테이블의 LOCAL_CODE)
ALTER TABLE DEPARTMENT ADD FOREIGN KEY(LOCATION_ID) REFERENCES LOCATION;

-- EMPLOYEE_COPY 테이블에 EMP_ID와 EMP_NO의 컬럼에 COMMENT 넣어주기
COMMENT ON COLUMN EMPLOYEE_COPY.EMP_ID IS '사번';
