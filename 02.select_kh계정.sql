...
--    단, 산술연산이 들어간것은 별칭부여
SELECT EMP_NAME, SALARY*12 연봉, (SALARY+BONUS*SALARY)*12 총수령액, (SALARY+BONUS*SALARY)*12*0.97 실수령액
FROM EMPLOYEE;
-- 2. LOCATION테이블에서 NATIONAL_CODE 옆에 '국가' 컬럼추가(별칭부여)
SELECT NATIONAL_CODE, '국가'
FROM LOCATION;
-- 3. DEPARTMENT테이블에서 1컬럼에 '인사관리부의 위치는 L1입니다'
...
-- EMPLOYEE에서 부서코드가 'D1'인 아닌 사원들의 사번, 사원명, 부서코드 조회
...
/*
    ...
    [표현법]
    비교대상컬럼 LIKE '특정패턴'
    : 특정패턴 제시할 때 '%', '_'를 와일드카드로 사용함
    
    >> '%' : 0글자 이상
    EX) 비교대상컬럼 LIKE '문자%' => 비교대상의 컬럼값이 '문자'로 시작하는것 조회
        비교대상컬럼 LIKE "%문자' => 비교대상의 컬럼값이 '문자'로 끝나는것 조회
        비교대상컬럼 LIKE '%문자%' => 비교대상의 컬럼값이 '문자'가 포함되는것 조회
        
    >> '_' : 1글자
    EX) 비교대상컬럼 LIKE '_문자' => 비교대상의 컬럼값이 '문자'앞에 무조건 한글자인 경우 조회
        비교대상컬럼 LIKE '__문자' => 비교대상의 컬럼값이 '문자'앞에 무조건 두글자인 경우 조회
        비교대상컬럼 LIKE '_문자_' => 비교대상의 컬럼값이 '문자'앞과 뒤에 무조건 한글자씩 있는 경우 조회
*/

-- EMPLOYEE테이블에서 사원들 중 성이 '전'씨인 사원들의 사원명, 급여, 입사일 조회
SELECT EMP_NAME, SALARY, HIRE_DATE
FROM EMPLOYEE
WHERE EMP_NAME LIKE '전%';

-- EMPLOYEE테이블에서 사원 이름중 '하'가 포함되어있는 사원명, 주민번호, 전화번호 조회
SELECT EMP_NAME, EMP_NO, PHONE
FROM EMPLOYEE
WHERE EMP_NAME LIKE '%하%';

-- EMPLOYEE테이블에서 사원이름 중 가운데 글자가 '하'인 사원들의 사원명, 전화번호 조회
SELECT EMP_NAME, PHONE
FROM EMPLOYEE
WHERE EMP_NAME LIKE '_하_';  
-- WHERE EMP_NAME LIKE '_하%';  -- 이름이 외자인 사원도 가능

-- EMPLOYEE테이블에서 전화번호 중 3번째 값이 '1'인 사원의 사원명, 전화번호, 이메일조회
-- (앞2글자 반드시 들어와야함, 4번째부터는 갯수와 상관없음)
SELECT EMP_NAME, PHONE, EMAIL
FROM EMPLOYEE
WHERE PHONE LIKE '__1%';

-- EMPLOYEE테이블에서 이메일 중 _앞의 글자가 3글자인 사원의 사번, 이름, 이메일조회
...
-- 모두다 와일드카드로 인식
-- > 어떤것이 와일드카드인지, 데이터값인지 구분해줘야 함
-- > 데이터값으로 취급하고 싶은 값 앞에 나만의 와일드카드(아무거나 문자, 숫자, 특수문자)를 제시하고 
--   나만의 와일드카드를 escape로 등록해야 함
SELECT EMP_ID, EMP_NAME, EMAIL
FROM EMPLOYEE
...
SELECT EMP_ID, EMP_NAME, EMAIL
FROM EMPLOYEE
WHERE NOT EMAIL LIKE '___d_%' ESCAPE 'd';
...
-- 3. EMPLOYEE테이블에서 이름에 '정보'가 포함되어 있는 사원들의 사원명, 급여 조회
...
-- EMPLOYEE테이블에서 보너스를 받지 않는 사원(BONUS값이 NULL)들의 사번, 이름, 급여, 보너스 조회
SELECT EMP_ID, EMP_NAME, SALARY, BONUS
FROM EMPLOYEE
-- WHERE BONUS = NULL;     -- 조회 안됨
WHERE BONUS IS NULL;

-- EMPLOYEE테이블에서 보너스를 받는 사원(BONUS값이 NULL)들의 사번, 이름, 급여, 보너스 조회
SELECT EMP_ID, EMP_NAME, SALARY, BONUS
FROM EMPLOYEE
WHERE BONUS IS NOT NULL;

-- EMPLOYEE테이블에서 사수가 없는 사원(MANAGER_ID가 NULL)들의 사원명, 사수사번, 부서코드 조회
SELECT EMP_NAME, MANAGER_ID, DEPT_CODE
FROM EMPLOYEE
WHERE MANAGER_ID IS NULL;

-- EMPLOYEE테이블에서 부서배치(DEPT_CODE)는 받지 않았지만 보너스는 받는 사원들의 이름, 보너스, 부서코드 조회
...
*/
-- ** OR보다 AND가 먼저 연산이 됨
-- EMPLOYEE테이블에서 직급코드가 J7이거나 J2인 사원들 중 급여가 200만원 이상인 사원들의 모든 컬럼 조회
SELECT *
FROM EMPLOYEE
...
/*
    <ORDER BY>
    SELECT 문 가장 마지막 줄에 작성 뿐만 아니라 실행순서 또한 마지막에 실행
    ...
    - NULLS FIRST : 정렬하고자하는 컬럼값에 NULL이 있으면 해당 데이터를 맨 앞에 배치(DESC 일때의 기본값)
    - NULLS LAST : 정렬하고자하는 컬럼값에 NULL이 있으면 해당 데이터를 맨 뒤에 배치(ASC 일때의 기본값)
*/

SELECT *
FROM EMPLOYEE
-- ORDER BY BONUS; -- 오름차순 정렬 NULL값이 LAST
-- ORDER BY BONUS DESC;    -- 내림차순 정렬 NULL값이 FIRST
...
SELECT *
FROM EMPLOYEE
ORDER BY BONUS DESC, SALARY ASC;    -- ASC 생략가능

-- 전 사원의 연봉별 내림차순 정렬 이때 NULL값은 맨 마지막에 나오도록 조회(이름, 연봉)
