...
-- 9. 한 사원과 같은 부서에서 일하는 사원의 이름 조회(본인제외)
SELECT E.EMP_NAME, E.DEPT_CODE, M.EMP_NAME, M.DEPT_CODE
...
--	13-3. WITH 사용
WITH SUM_SAL AS (SELECT DEPT_TITLE, SUM(SALARY)
                    FROM EMPLOYEE
                    JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
                    GROUP BY DEPT_TITLE
                    HAVING SUM(SALARY) > (SELECT SUM(SALARY)*0.2
...
WITH SUMAVG_SAL AS (SELECT SUM(SALARY) 급여합, CEIL(AVG(SALARY)) 급여평균
                    FROM EMPLOYEE
                    GROUP BY DEPT_CODE)
SELECT *
FROM SUMAVG_SAL;